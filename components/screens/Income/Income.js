...
function Income() {
    ...
    useEffect(() => {
        setloading(true);
        setUser((auth()._user.email.split("@")[0] + "-Income").toString());
        getDoc();
    }, [navigation])
    ...
    const searchFilterFunction = (text) => {
        if (text) {
            const newData = IncomeData.filter(function (item) {
            ...
            setFilteredDataSource(IncomeData);
            setSearch(text);
        }
    };
    ...
    const getItem = (item) => {
        navigation.navigate('IncomeList', { Id: item.Id, UserId: (auth()._user.email.split("@")[0] + "-Income").toString(), Icon: item.Icon, Iconfamily: item.IconFamily });
    };

    return (
        <SafeAreaView style={{ backgroundColor: "#1C1C1C" }} width="100%" height="100%">
            <View style={{ width: "90%", paddingTop: "10%", paddingBottom: "8%" }}>
                <Text style={{ fontSize: 30, fontWeight: "bold", color: "white", paddingLeft: 15 }}>Income</Text>
            </View>
